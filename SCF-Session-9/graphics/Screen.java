...
public class Screen {
	...
	public boolean addShape(Shape shapeObject) {
		try {
			...
			if (shapeOrigin.getXCoordinate() < 0
					|| shapeOrigin.getXCoordinate() > xMax
					|| shapeOrigin.getYCoordinate() < 0
					|| shapeOrigin.getYCoordinate() > yMax) {
	 ...
	 * @return true if removed, false otherwise
	 */
	public boolean deleteShapeType(ShapeEnum shapeType) {
		boolean flag = false;
	...
	/**
	 * sorts the list of objects in ascending order according to distance
	 * between origin of shape and origin of screen
	 * 
	 * @return sortedList
	 */
	public ArrayList<Shape> sortByOriginDistance() {
		int noOfObjects = screenObjects.size();
		Shape temp;
		ArrayList<Shape> sortedListByOriginDistance = new ArrayList<Shape>(
				screenObjects);
		...
		for (int i = 0; i < noOfObjects; i++) {
			for (int j = 0; j < noOfObjects - 1 - i; j++) {
				...
				if (sortedListByOriginDistance.get(j).getOriginDistance() > sortedListByOriginDistance
						.get(j + 1).getOriginDistance()) {
					temp = sortedListByOriginDistance.get(j);
					sortedListByOriginDistance.set(j,
							sortedListByOriginDistance.get(j + 1));
