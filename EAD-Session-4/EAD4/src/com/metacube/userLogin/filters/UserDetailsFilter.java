...
public class UserDetailsFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		String messageToShowOnView = getValidationMessage(firstName, lastName,
		...
		if ("".equals(messageToShowOnView)) {
			...
			RequestDispatcher RequetsDispatcherObj = request
					.getRequestDispatcher("/register_user.jsp");
	...
	private String getValidationMessage(String firstName, String lastName,
			String contact, String password, String confirmPassword) {
		String message = "";
		if (!checkName(firstName)) {
			message = message
					+ "\n ERROR : first name should be alphabate and minimum of 3 character: ";
		}
		if (!checkName(lastName)) {
			message = message
					+ "\n ERROR : last name should be alphabate and minimum of 3 character: ";
		}
		if (!checkContact(contact)) {
			message = message
					+ "\n ERROR : invalid contact 10 digits are required: ";
		}
		if (!checkPassword(password)) {
			message = message
					+ "\n ERROR : password should be alphanumeric EG: Demo@1234 and consist minimum 8 character: ";
		}
